...
import { createDonation as apiCreateDonation } from '../lib/donations/api-donations';
...
export function DonationForm({ campaignId, campaignTitle }: DonationFormProps) {
  ...
  const [method, setMethod] = useState<'bank_card' | 'sbp'>('bank_card');
  ...
  const displayName = useMemo(() => {
    if (anonymous) return 'Анонимная помощь';
    return donorName.trim() || 'Гость';
  }, [anonymous, donorName]);

  async function submitDonation(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!accepted) { setShowAcceptError(true); return; }
    if (amount < 10) return;
    if (!canSubmit) return;

    setIsSubmitting(true);
    const phoneNormalized = phone.replace(/[^\d+]/g, '').slice(0, 16);
    const result = await apiCreateDonation({
      campaignId,
      campaignTitle,
      amount,
      ...
      anonymous,
      method,
      userPhone: phoneNormalized,
    });
    if (result.success) {
      setDonationId(result.donationId || `don_${Date.now()}`);
    }
    ...
    <form className="rounded-[28px] bg-white p-5 shadow-[0_24px_70px_rgb(7_17_31/10%)] md:p-7" onSubmit={submitDonation}>
      ...
      <div className="mt-4 rounded-[22px] border border-zinc-200 bg-white px-4 py-3">
        <div className="flex items-center gap-3">
          <input aria-label="Сумма пожертвования" className="min-w-0 flex-1 bg-transparent text-4xl font-semibold tracking-tight outline-none" min="10" onChange={(e) => setAmount(Number(e.target.value))} type="number" value={amount} />
          <span className="text-4xl font-semibold">₽</span>
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          {quickAmounts.map((qa) => (
            <button className={`rounded-full px-4 py-1.5 text-sm font-black ${amount === qa ? 'bg-[#07111f] text-white' : 'bg-[#f4f5f7] text-[#07111f]'}`} key={qa} onClick={() => setAmount(qa)} type="button">{formatRub(qa)}</button>
          ))}
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <button className={`flex h-16 items-center gap-3 rounded-[20px] border px-4 text-left text-base font-black ${method === 'bank_card' ? 'border-[#dceee5] bg-[#eef6f2]' : 'border-zinc-200 bg-white'}`} onClick={() => setMethod('bank_card')} type="button">
          <span className="grid h-9 w-9 place-items-center rounded-xl bg-[#20b89a] text-white">▬</span>Картой онлайн
        </button>
        <button className={`flex h-16 items-center gap-3 rounded-[20px] border px-4 text-left text-base font-black ${method === 'sbp' ? 'border-[#dceee5] bg-[#eef6f2]' : 'border-zinc-200 bg-white'}`} onClick={() => setMethod('sbp')} type="button">
          <span className="grid h-9 w-9 place-items-center rounded-xl bg-white text-lg">△</span>Через СБП
        </button>
      </div>
